...
from QMaterial.Widget.Button import FlatButton
...
class Window(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for row, key in enumerate(colors):
            layout.addWidget(FlatButton(key, self), row, 0, 1, 1)
            for column, value in enumerate(colors.get(key)):
                b = FlatButton(value._name, self)
...
if __name__ == "__main__":
    ...
    app.setApplicationDisplayName("FlatButton")
    app.setApplicationName("FlatButton")
    window = QScrollArea()
