...
@file: test.TestButtonRaisedRipple
@description: 
'''
import os
import sys
...
from QMaterial.Widget.Button import RippleRaisedButton
...
class Window(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for row, key in enumerate(colors):
            layout.addWidget(RippleRaisedButton(key, self), row, 0, 1, 1)
            for column, value in enumerate(colors.get(key)):
                b = RippleRaisedButton(value._name, self)
...
if __name__ == "__main__":
    ...
    app.setApplicationDisplayName("RippleRaisedButton")
    app.setApplicationName("RippleRaisedButton")
    window = QScrollArea()
